...
/*   Updated: 2021/03/07 23:47:40 by taeheoki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_cor_row_index(int i, int j, int *a, int *b)
{
	if (g_arr[i][j] < g_arr[i][j + 1] && g_arr[i][j + 1] > *a)
	{
		g_count_1++;
		*a = g_arr[i][j + 1];
	}
	if (g_arr[i][3 - j] < g_arr[i][2 - j] && g_arr[i][2 - j] > *b)
	{
		g_count_2++;
		*b = g_arr[i][2 - j];
	}
